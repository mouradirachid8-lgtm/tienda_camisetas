...
<body class="bg-gray-100 font-sans">
  ...
  <div class="container mx-auto px-4 py-12 max-w-6xl">
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
      ...
      @else
        <div
        class="bg-gradient-to-r from-gray-400 to-gray-500 text-white p-12 rounded-xl text-center shadow-inner flex items-center justify-center h-full">
        <div>
      ...
      @endif
        ...
        <span
          class="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-sm">{{ $producto->equipo->nombre }}</span>
        </p>
      @endif

          <div class="flex items-center mb-6 border-b border-gray-100 pb-4">
            @if($producto->descuento > 0)
        <h3 class="text-3xl font-bold text-red-600 mr-3">{{ number_format($producto->aplicar_descuento(), 2) }}€</h3>
        <h5 class="text-lg text-gray-400 line-through">{{ number_format($producto->precio, 2) }}€</h5>
        <span
          class="ml-3 bg-red-600 text-white px-3 py-1 text-xs font-bold rounded-full">-{{ $producto->descuento }}%</span>
      @else
        <h3 class="text-3xl font-bold text-gray-800">{{ number_format($producto->precio, 2) }}€</h3>
      @endif
          </div>
        ...
        <span
          class="inline-flex items-center bg-green-100 text-green-800 px-3 py-1 text-sm font-medium rounded-full">
      ...
      @endif
          ...
          @if($producto->es_disponible())
          <form action="{{ route('carrito.agregarAlCarrito', $producto->id) }}" method="POST" class="mt-6">
          @csrf
      ...
      @else
        <div class="mt-6">
        <button type="button"
          class="w-full bg-gray-200 text-gray-500 font-medium py-3 px-6 rounded-lg cursor-not-allowed">
          Producto no disponible
        </button>
        <p class="text-sm text-gray-500 mt-2">Este producto está temporalmente agotado. Puedes añadirlo a tu lista
          de deseos para recibir una notificación cuando esté disponible.</p>
        </div>
      @endif
        </div>
      </div>
